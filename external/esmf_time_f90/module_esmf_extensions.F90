...
! 5.2.0r      type(ESMF_CalendarType), intent(out), optional :: calendarType
      ...
      INTEGER(ESMF_KIND_I4) :: year, seconds, Sn1, Sd1
...
! 5.2.0r                                    calendar=calendar, calendarType=calendarType, timeZone=timeZone, &
      ...
      IF ( rc == ESMF_SUCCESS ) THEN
        IF ( PRESENT( dayOfYear_r8 ) ) THEN
          ...
          CALL ESMF_TimeGet( time, yy=year, s=seconds, sN=Sn1, sD=Sd1, rc=rc )
          IF ( rc == ESMF_SUCCESS ) THEN
            ...
            IF ( PRESENT( Sd ) ) THEN
              IF ( Sd /= 0 ) THEN
                rsec = rsec + ( REAL( Sn1, ESMF_KIND_R8 ) / REAL( Sd1, ESMF_KIND_R8 ) )
              ENDIF
            ENDIF
