!
!==============================================================================
!
!     CESMF Calendar Module
      module CESMF_CalendarMod
!
!==============================================================================
!
! This file contains the Calendar class definition and all Calendar class
! methods.
!
!------------------------------------------------------------------------------
! INCLUDES
#include <CESMF_TimeMgr.inc>

!==============================================================================
!BOPI
! !MODULE: CESMF_CalendarMod
!
! !DESCRIPTION:
! Part of Time Manager F90 API wrapper of C++ implemenation
!
! Defines F90 wrapper entry points for corresponding
! C++ class { \tt ESMC\_Calendar} implementation
!
! See {\tt ../include/ESMC\_Calendar.h} for complete description
!
!------------------------------------------------------------------------------
! !USES:
      ! inherit from CESMF base class
      use CESMF_BaseMod

      ! inherit from base time class
      use CESMF_BaseTimeMod

      implicit none
!
!------------------------------------------------------------------------------
! !PRIVATE TYPES:
      private
!------------------------------------------------------------------------------



      INTEGER, PARAMETER :: MONTHS_PER_YEAR = 12
      INTEGER, PARAMETER :: mday(MONTHS_PER_YEAR)   &
                          = (/31,28,31,30,31,30,31,31,30,31,30,31/)
      INTEGER, PARAMETER :: mdayleap(MONTHS_PER_YEAR) &
                          = (/31,29,31,30,31,30,31,31,30,31,30,31/)
      INTEGER, DIMENSION(365) :: daym
      INTEGER, DIMENSION(366) :: daymleap
      INTEGER :: mdaycum(0:MONTHS_PER_YEAR)
      INTEGER :: mdayleapcum(0:MONTHS_PER_YEAR)
      TYPE(CESMF_BaseTime), TARGET :: monthbdys(0:MONTHS_PER_YEAR)
      TYPE(CESMF_BaseTime), TARGET :: monthbdysleap(0:MONTHS_PER_YEAR)


!------------------------------------------------------------------------------
!     ! CESMF_CalendarType
!
!     ! F90 "enum" type to match C++ ESMC_CalendarType enum

      type CESMF_CalendarType
      private
        integer :: caltype
      end type

      type(CESMF_CalendarType), parameter :: &
                               CESMF_CAL_GREGORIAN =  CESMF_CalendarType(1), &
                               CESMF_CAL_JULIAN =     CESMF_CalendarType(2), &
                           ! like Gregorian, except Feb always has 28 days
                               CESMF_CAL_NOLEAP =     CESMF_CalendarType(3), & 
                           ! 12 months, 30 days each
                               CESMF_CAL_360DAY =     CESMF_CalendarType(4), & 
                           ! user defined
                               CESMF_CAL_GENERIC =    CESMF_CalendarType(5), &
                           ! track base time seconds only
                               CESMF_CAL_NOCALENDAR = CESMF_CalendarType(6)

!------------------------------------------------------------------------------
!     ! CESMF_Calendar
!
!     ! F90 class type to match C++ Calendar class in size only;
!     !  all dereferencing within class is performed by C++ implementation
!
!------------------------------------------------------------------------------
!
!     ! CESMF_DaysPerYear
!
      type CESMF_DaysPerYear
      private
        integer :: D        ! whole days per year
! Fractional days-per-year are not yet used in this implementation.  
!        integer :: Dn       ! fractional days per year numerator
!        integer :: Dd       ! fractional days per year denominator
      end type              ! e.g. for Venus, D=0, Dn=926, Dd=1000
!
!------------------------------------------------------------------------------
!     ! CESMF_Calendar
!
!
      type CESMF_Calendar
      private
        type(CESMF_CalendarType) :: Type
! TBH:  When NO_DT_COMPONENT_INIT is set, code that uses F95 compile-time 
! TBH:  initialization of components of derived types is not included.  
! TBH:  Some older compilers, like PGI 5.x do not support this F95 feature.  
#ifdef NO_DT_COMPONENT_INIT
        logical :: Set
#else
        logical :: Set = .false.
#endif
        integer, dimension(MONTHS_PER_YEAR) :: DaysPerMonth
        integer :: SecondsPerDay
        integer :: SecondsPerYear
        type(CESMF_DaysPerYear) :: DaysPerYear
      end type

!------------------------------------------------------------------------------
! !PUBLIC DATA:
   TYPE(CESMF_Calendar), public, save, pointer :: defaultCal   ! Default Calendar


!
!------------------------------------------------------------------------------
! !PUBLIC TYPES:
      public MONTHS_PER_YEAR
      public mday
      public mdayleap
      public monthbdys
      public monthbdysleap
      public daym
      public daymleap
      public mdaycum
      public mdayleapcum
      public CESMF_CalendarType
      public CESMF_CAL_GREGORIAN, CESMF_CAL_NOLEAP, &
             CESMF_CAL_360DAY, CESMF_CAL_NOCALENDAR
!      public CESMF_CAL_JULIAN
!      public CESMF_CAL_GENERIC
      public CESMF_Calendar

!------------------------------------------------------------------------------
!
! !PUBLIC MEMBER FUNCTIONS:
      public CESMF_CalendarCreate

! Required inherited and overridden CESMF_Base class methods

      public CESMF_CalendarInitialized ! Only in this implementation, intended
                                      ! to be private within CESMF methods
!EOPI

!==============================================================================

      contains


!==============================================================================
!BOP
! !IROUTINE: CESMF_CalendarCreate - Create a new CESMF Calendar of built-in type

! !INTERFACE:
      ! Private name; call using CESMF_CalendarCreate()
      function CESMF_CalendarCreate(name, calendartype, rc)

! !RETURN VALUE:
      type(CESMF_Calendar) :: CESMF_CalendarCreate

! !ARGUMENTS:
      character (len=*),       intent(in),  optional :: name
      type(CESMF_CalendarType), intent(in)            :: calendartype
      integer,                 intent(out), optional :: rc

! !DESCRIPTION:
!     Creates and sets a {\tt calendar} to the given built-in
!     {\tt CESMF\_CalendarType}. 
!
!     This is a private method; invoke via the public overloaded entry point
!     {\tt CESMF\_CalendarCreate()}.
!
!     The arguments are:
!     \begin{description}
!     \item[{[name]}]
!          The name for the newly created calendar.  If not specified, a
!          default unique name will be generated: "CalendarNNN" where NNN
!          is a unique sequence number from 001 to 999.
!     \item[calendartype]
!          The built-in {\tt CESMF\_CalendarType}.  Valid values are:
!            {\tt CESMF\_CAL\_360DAY}, {\tt CESMF\_CAL\_GREGORIAN},
!            {\tt CESMF\_CAL\_JULIANDAY}, {\tt CESMF\_CAL\_NOCALENDAR}, and
!            {\tt CESMF\_CAL\_NOLEAP}.
!          See the "Time Manager Reference" document for a description of
!          each calendar type.
!     \item[{[rc]}]
!          Return code; equals {\tt CESMF\_SUCCESS} if there are no errors.
!     \end{description}
!    
!EOP
! !REQUIREMENTS:
!     TMGn.n.n
      type(CESMF_DaysPerYear) :: dayspy

      if ( present(rc) ) rc = CESMF_FAILURE
! Calendar type is hard-coded.  Use CESMF library if more flexibility is 
! needed.  
#ifdef NO_LEAP_CALENDAR
      if ( calendartype%caltype  /= CESMF_CAL_NOLEAP%caltype ) then
         write(6,*) 'Not a valid calendar type for this implementation'
         write(6,*) 'This implementation only allows CESMF_CAL_NOLEAP'
         write(6,*) 'calender type set to     = ', calendartype%caltype
         write(6,*) 'NO_LEAP calendar type is = ', CESMF_CAL_NOLEAP%caltype
         return
      end if
      CESMF_CalendarCreate%Type = CESMF_CAL_NOLEAP
#else
      if ( calendartype%caltype  /= CESMF_CAL_GREGORIAN%caltype ) then
         write(6,*) 'Not a valid calendar type for this implementation'
         write(6,*) 'This implementation only allows CESMF_CAL_GREGORIAN'
         write(6,*) 'calender type set to     = ', calendartype%caltype
         write(6,*) 'GREGORIAN calendar type is = ', CESMF_CAL_GREGORIAN%caltype
         return
      end if
      CESMF_CalendarCreate%Type = CESMF_CAL_GREGORIAN
#endif
! This is a bug on some systems -- need initial value set by compiler at 
! startup.  
! However, note that some older compilers do not support compile-time 
! initialization of data members of Fortran derived data types.  For example, 
! PGI 5.x compilers do not support this F95 feature.  See 
! NO_DT_COMPONENT_INIT.  
      CESMF_CalendarCreate%Set = .true.
      CESMF_CalendarCreate%SecondsPerDay = SECONDS_PER_DAY
! DaysPerYear and SecondsPerYear are incorrect for Gregorian calendars...  
      dayspy%D = size(daym)
!TBH:  TODO:  Replace DaysPerYear and SecondsPerYear with methods 
!TBH:  TODO:  since they only make sense for the NO_LEAP calendar!  
      CESMF_CalendarCreate%DaysPerYear = dayspy
      CESMF_CalendarCreate%SecondsPerYear = CESMF_CalendarCreate%SecondsPerDay &
                                       * dayspy%D
!TBH:  TODO:  use mdayleap for leap-year calendar
      CESMF_CalendarCreate%DaysPerMonth(:) = mday(:)

      if ( present(rc) ) rc = CESMF_SUCCESS

      end function CESMF_CalendarCreate


!==============================================================================
!BOP
! !IROUTINE: CESMF_CalendarInitialized - check if calendar was created

! !INTERFACE:
      function CESMF_CalendarInitialized(calendar)

! !RETURN VALUE:
      logical CESMF_CalendarInitialized

! !ARGUMENTS:
      type(CESMF_Calendar), intent(in)            :: calendar

! !DESCRIPTION:
!EOP
! !REQUIREMENTS:
!     TMGn.n.n
! Note that return value from this function will be bogus for older compilers 
! that do not support compile-time initialization of data members of Fortran 
! derived data types.  For example, PGI 5.x compilers do not support this F95 
! feature.  At the moment, the call to this fuction is #ifdefd out when the 
! leap-year calendar is used so this is not an issue for WRF (see 
! NO_DT_COMPONENT_INIT).  
        CESMF_CalendarInitialized = calendar%set

     end function CESMF_CalendarInitialized

      end module CESMF_CalendarMod
